...
class DuckieListener:
    ...
    def listen_to_external_location(self):
        # Set up the listener node in a separate process
        ...
        rospy.spin()  # Keep this process alive to listen for messages
    ...
    def send_local_location(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.location_received_event.wait():  # This will block until the event is set
                ...
                self.location_received_event.clear()  # Reset event for next location update
...
# class DuckieListenerNode:
#     def __init__(self):
#         # os.environ["ROS_MASTER_URI"] = "http://192.168.158.187:11311"  # Duckiebot's ROS master
#         rospy.init_node('duckie_listener_node',anonymous=True)
#         # Subscribe to the Duckiebot's location topic
#         vehicle_name = rospy.get_param("~vehicle_name", "duckie1")
#         external_location_topic = '/{}/location'.format(vehicle_name)
#         self.location_sub = rospy.Subscriber(external_location_topic, String, self.location_callback)

#         # Publish to a local ROS topic
#         # self.local_pub = rospy.Publisher('/local_location', String, queue_size=10)
#         rospy.loginfo("Made it here ")

#     def location_callback(self, msg):
#         # Forward the location data to the local topic
#         rospy.loginfo("Received location: %s", msg.data)
